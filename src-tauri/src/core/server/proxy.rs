...
pub struct ProxyConfig {
    ...
    /// Local engine ("llamacpp" | "mlx" | "mistralrs") that wins when a
    /// request names no model and several engines have one loaded.
...
use crate::core::server::LocalSessions;
...
async fn resolve_upstream_for_model(
    ...
    local_sessions: Arc<LocalSessions>,
    ...
    let mlx_guard = local_sessions.mlx.lock().await;
    ...
    let mistralrs_guard = local_sessions.mistralrs.lock().await;
    if let Some(session) = mistralrs_guard
        .values()
...
async fn first_available_local_model_id(
    ...
    // An explicit provider preference wins when that engine has a model up.
    match preferred_provider {
        Some("mlx") => {
            let mlx_guard = local_sessions.mlx.lock().await;
            if let Some(m) = mlx_guard.values().next().map(|s| s.info.model_id.clone()) {
                return Some(m);
            }
        }
        Some("mistralrs") => {
            let mistralrs_guard = local_sessions.mistralrs.lock().await;
            if let Some(m) = mistralrs_guard
                .values()
                .next()
...
/// mistralrs-server only accepts its model path or "default" as the request
/// model name. Each session serves exactly one model, so when `model_id`
/// belongs to a mistralrs session the upstream request must say "default".
async fn upstream_request_model_name(
    local_sessions: &Arc<LocalSessions>,
    model_id: &str,
) -> String {
    let mistralrs_guard = local_sessions.mistralrs.lock().await;
...
async fn resolve_proxy_model_id(
    ...
    let fallback_model = first_available_local_model_id(
        llama_state,
        client,
        local_sessions,
        preferred_provider,
...
async fn run_server_side_openai_orchestration(
    ...
    let upstream_model_name =
        upstream_request_model_name(&local_sessions, &model_id).await;

    for _turn in 0..max_turns {
        let mut completion_map = serde_json::Map::new();
        completion_map.insert("model".to_string(), serde_json::json!(upstream_model_name));
...
async fn proxy_request(
    ...
    match (method.clone(), destination_path.as_str()) {
                ...
                Ok(json_body) => {
                    ...
                    if let Some(model_id) = json_body.get("model").and_then(|v| v.as_str()) {
                        ...
                        if let Some(ref p) = provider_name {
                            ...
                            let mistralrs_port = {
                                let mistralrs_guard =
                                    local_sessions.mistralrs.lock().await;
                                mistralrs_guard
                                    .values()
                                    .find(|s| s.info.model_id == model_id)
                            ...
                            } else if let Some((target_port, api_key)) = mistralrs_port {
                                session_api_keys = vec![api_key];
                                target_base_url =
                                    Some(format!("http://127.0.0.1:{}/v1/messages", target_port));
        ...
        (hyper::Method::POST, "/orchestrations") => {
            ...
            if let Some(ov) = model_override {
                ...
                if !trimmed.is_empty() && trimmed != "*" {
                    ...
                    model_id = first_available_local_model_id(
                        llama_state.clone(),
                        &client,
                        local_sessions.clone(),
                        config.preferred_local_provider.as_deref(),
                    )
                    .await;
                }
            } else {
            ...
            for _turn in 0..max_turns {
                ...
                let mut completion_map = serde_json::Map::new();
                completion_map
                    .insert("model".to_string(), serde_json::json!(upstream_model_name));
                completion_map.insert(
                    "messages".to_string(),
                    serde_json::Value::Array(conversation_messages.clone()),
                );
                ...
                Ok(mut json_body) => {
                    ...
                    if let Some(model_id) = json_body.get("model").and_then(|v| v.as_str()) {
                        ...
                        if let Some(ref provider) = provider_name {
                            ...
                            if mlx_session_info.is_none()
                                && mistralrs_session.is_none()
                                && router_up.is_none()
                            {
                                log::warn!(
                            ...
                            } else if let Some((target_port, api_key)) = mistralrs_session {
                                session_api_keys = vec![api_key];
                                log::debug!(
                                    "Found mistral.rs session for model_id {model_id}"
                                );
                                target_base_url = Some(format!(
                                    "http://127.0.0.1:{target_port}/v1{destination_path}"
                                ));
                                // mistralrs-server rejects unknown model names;
                                // a session serves one model, so "default" it is.
                                json_body["model"] = serde_json::json!("default");
                                if let Ok(rewritten) = serde_json::to_vec(&json_body) {
                                    buffered_body = Some(rewritten.into());
                                }
        ...
        (hyper::Method::GET, "/models") => {
            ...
            let mistralrs_models: Vec<_> = {
                ...
                mistralrs_guard
                    ...
                    .map(|session| {
                        ...
                        // mistral.rs shares the llamacpp model directory.
                        let yml_path = std::path::Path::new(&jan_data_folder)
                            .join("llamacpp")
                            .join("models")
                            .join(&id)
            ...
            let mistralrs_count = mistralrs_models.len();
            let remote_count = remote_models.len();

            // Combine all models
            let mut all_models = Vec::with_capacity(
                local_count + mlx_count + mistralrs_count + remote_count,
            );
            all_models.extend(local_models);
            all_models.extend(mlx_models);
            all_models.extend(mistralrs_models);
            ...
            log::debug!(
                "Returning {} models ({} llama.cpp, {} MLX, {} mistral.rs, {} remote)",
                all_models.len(),
                local_count,
                mlx_count,
                mistralrs_count,
...
pub async fn start_server(
    ...
    preferred_local_provider: Option<String>,
