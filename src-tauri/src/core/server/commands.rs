...
pub struct StartServerConfig {
    ...
    /// Which local engine should serve requests that don't name a model
    /// when several engines have one loaded ("llamacpp" | "mlx" | "mistralrs").
...
pub async fn start_server<R: Runtime>(
    ...
    let mistralrs_sessions = {
        let mistralrs_state: State<MistralrsState> = app_handle.state();
        mistralrs_state.server_processes.clone()
    };
